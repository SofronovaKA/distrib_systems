...
## Расчет пропускной способности системы (Bandwidth)
...
|----------|----------|-----------|
| PacketSize | 1024 байт | Типичный размер gossip-сообщения |
| Overhead | 1.2 | 20% накладных расходов на заголовки |
| 8 | — | перевод байт в биты |

### Параметры системы (вариант 17)

| Параметр | Значение |
|----------|----------|
| Gossip Interval | 0.2 с |
| Базовый Fanout | 3 (адаптивный: 3 → до 10) |
| Количество узлов (N) | 100 |
| Отказы узлов (Failures) | 5% |
| Активные узлы | 100 × (1 − 0.05) = 95 |
...
Потери пакетов не снижают отправленный трафик — узлы всё равно отправляют сообщения. Потери только уменьшают количество успешных доставок, что в симуляции приводит к росту времени конвергенции.

| Packet Loss | Отправлено (Мбит/с) | Эффективно доставлено (Мбит/с) |
|-------------|---------------------|--------------------------------|
| 0% | 14.0 | 14.0 |
| 5% | 14.0 | 13.3 |
| 10% | 14.0 | 12.6 |
...
### Расчёт для адаптивного Fanout (при потерях 20%)
...
| Режим | Fanout | Потери | Отправлено (Мбит/с) | Примечание |
|-------|--------|--------|---------------------|------------|
| Фиксированный | 3 | 0–20% | 14.0 | Не зависит от потерь |
| Адаптивный | 3 (норма) | 0–5% | 14.0 | Как фиксированный |
| Адаптивный | 4–5 | 10% | 18.7–23.3 | Умеренный рост |
| Адаптивный | 6–8 | 20% | 28.0–37.3 | Значительный рост |

### Вывод по пропускной способности

1. **Фиксированный режим (Fanout=3):** 14 Мбит/с постоянно.
2. **Адаптивный режим при 20% потерь (Fanout≈6–8):** 28–37 Мбит/с.
3. **Цена адаптации:** трафик растёт линейно с Fanout. При высоких потерях это позволяет системе оставаться отзывчивой.
...
[Открыть ноутбук в Google Colab](https://colab.research.google.com/drive/твоя_ссылка)
